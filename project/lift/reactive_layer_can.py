...
from robosuite.project.lift.kalman_filter import *
from robosuite.project.lift.coulomb_controller import *
...
class ReactiveLayer():
    ...
    # function to compute the vertical compensation force
    def exert_w_force(self, robots, mat0, mat1):
        z = np.array([0,0,1])
        f0 = np.array(z*0.011)
        f1 = np.array(z*0.011)
        forces_d=  np.array([f0, f1])
        torques_d= np.array([[0,0,0], [0,0,0]])
        act = desired_force_to_torque_W(robots, mat0, mat1, forces_d, torques_d)
        return act
    ...
    def exert_force(self, robots, mat0, mat1):
        forces_d=  np.array([self.f0, self.f1])
        torques_d= np.array([[0,0,0], [0,0,0]])
        self.act = desired_force_to_torque_W(robots, mat0, mat1, forces_d, torques_d) + self.exert_w_force(robots, mat0, mat1)
        self.act = desired_force_to_torque_EEF(robots, mat0, mat1, forces_d, torques_d) + self.exert_w_force(robots, mat0, mat1)
        
    # function to compute the action to exert in environment
    def action(self, robots, mat0, mat1, tgf0, tgf1, n0, n1):
        self.measure_tangential_force(tgf0, tgf1)
        self.estimate_actuation_direction(n0, n1)
        self.compute_force()
        self.exert_force(robots, mat0, mat1)
        return self.act, self.f0, self.f1, self.ReL_intervention, self.e0, self.e1
